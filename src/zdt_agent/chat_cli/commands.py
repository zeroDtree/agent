...
APPROVAL_SUMMARIES = {
    ToolApprovalPolicy.MANUAL: "confirm every tool call",
    ToolApprovalPolicy.BLACKLIST_REJECT: "auto-reject writes; confirm others",
    ToolApprovalPolicy.UNIVERSAL_REJECT: "block all tools",
    ToolApprovalPolicy.WHITELIST_ACCEPT: "auto-approve reads; confirm writes",
    ToolApprovalPolicy.UNIVERSAL_ACCEPT: "fully automatic",
}

NETWORK_SUMMARIES = {
    "on": "allow outbound connections for tools tagged needs_network",
    "off": "block tools tagged needs_network",
}

HELP_TEXT = (
    "Commands:\n"
    "  !mode                        - show current work mode\n"
    "  !mode list                   - list available work modes\n"
    "  !mode ro|sw|aw|pl            - switch work mode\n"
    "  !network                     - show network switch state\n"
    "  !network list                - list network switch options\n"
    "  !network on|off              - enable/disable outbound network\n"
    "  !approval                    - show tool approval policy\n"
    "  !approval list               - list tool approval policies\n"
    "  !approval <policy>           - switch tool approval policy\n"
    ...
    "Tip: MCP tools require '!network on' when network is off (see config/mcp_tools/default.yaml).\n"
)
...
class CommandDispatcher:
    ...
    async def _handle_mode(self, parts: list[str]) -> None:
        ...
        if target == "list":
            print("Available work modes:")
            for mode in WorkMode:
                marker = " (active)" if mode == work_config.work_mode else ""
                summary = MODE_SUMMARIES.get(mode, "")
                suffix = f" — {summary}" if summary else ""
                print(f"  {mode.value}{marker}{suffix}")
            return
        try:
            new_mode = WorkMode(target)
        except ValueError:
            print("Usage: !mode list|ro|sw|aw|pl")
            return
    ...
    async def _handle_network(self, parts: list[str]) -> None:
        ...
        target = parts[1].strip().lower()
        if target == "list":
            current = "on" if work_config.network.enabled else "off"
            print("Network switch options:")
            for option in ("on", "off"):
                marker = " (active)" if option == current else ""
                summary = NETWORK_SUMMARIES.get(option, "")
                suffix = f" — {summary}" if summary else ""
                print(f"  {option}{marker}{suffix}")
            return
        if target not in {"on", "off"}:
            print("Usage: !network list|on|off")
            return
        if target == "on":
    ...
    async def _handle_approval(self, parts: list[str]) -> None:
        work_config = self.state.work_config
        if work_config is None:
            print("Tool approval policy is not available in this session.")
            return
        if len(parts) == 1:
            print(f"Tool approval: {work_config.tool_approval.value}")
            self._print_approval_summary(work_config.tool_approval)
            return
        target = parts[1].strip().lower()
        if target == "list":
            print("Available approval policies:")
            for policy in ToolApprovalPolicy:
                marker = " (active)" if policy == work_config.tool_approval else ""
                summary = APPROVAL_SUMMARIES.get(policy, "")
                suffix = f" — {summary}" if summary else ""
                print(f"  {policy.value}{marker}{suffix}")
            return
        try:
            new_policy = ToolApprovalPolicy(target)
        except ValueError:
            options = "|".join(policy.value for policy in ToolApprovalPolicy)
            print(f"Usage: !approval list|{options}")
            return
        if new_policy == work_config.tool_approval:
            print(f"Tool approval already set to: {new_policy.value}")
            return
        if new_policy in {ToolApprovalPolicy.UNIVERSAL_ACCEPT, ToolApprovalPolicy.WHITELIST_ACCEPT}:
            prompt = (
                "Universal accept runs all tools without confirmation. Continue? (yes/no): "
                if new_policy == ToolApprovalPolicy.UNIVERSAL_ACCEPT
                else "Whitelist accept auto-approves read tools. Continue? (yes/no): "
            )
            confirm = input(prompt)
            if confirm.strip().lower() not in {"y", "yes"}:
                print("Tool approval switch cancelled.")
                return
        work_config.tool_approval = new_policy
        print(f"Tool approval switched to: {new_policy.value}")
        self._print_approval_summary(new_policy)

    def _print_approval_summary(self, policy: ToolApprovalPolicy) -> None:
        print(APPROVAL_SUMMARIES.get(policy, ""))

    def _print_mode_summary(self, work_config) -> None:
        print(MODE_SUMMARIES.get(work_config.work_mode, ""))
