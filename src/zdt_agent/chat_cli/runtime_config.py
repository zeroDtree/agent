...
from ..paths import prompts_dir, runtime_root
...
def resolve_prompt_dir(prompt_dir: str) -> Path:
    relative = prompt_dir.strip().strip("/")
    candidate = Path(relative).expanduser()
    if candidate.is_absolute():
        return candidate.resolve()
    return (prompts_dir() / relative).resolve()
...
def resolve_role_prompt_path(prompt_dir: Path, role_name: str) -> str:
    role_path = (prompt_dir / f"{role_name}.md").resolve()
    prompts_root = prompts_dir().resolve()
    try:
        return str(role_path.relative_to(prompts_root))
    except ValueError:
...
def resolve_conversation_dir(cfg: DictConfig) -> Path:
    ...
    if not conversation_dir.is_absolute():
        conversation_dir = (runtime_root() / conversation_dir).resolve()
...
def build_chat_session_state(
    ...
    prompt_dir = resolve_prompt_dir(str(char_cfg.get("prompt_dir", "chars")) if char_cfg else "chars")
